...
static void json_restore_call(JsonParser *parser, const str *id, int foreign);
...
static void json_restore_call(JsonParser *parser, const str *callid, int foreign) {
	...
	if (redis_hash_get_timeval(&last_signal, &call, "last_signal"))
		goto err3;

	if (c->last_signal.tv_sec) {
		err = NULL;
		// is the call we're loading newer than the one we have?
		if (last_signal > c->last_signal.tv_sec) {
			// switch ownership
			call_make_own_foreign(c, foreign);
			c->last_signal.tv_sec = last_signal;
		}
...
static int redis_update_call_tags(struct call *c, redis_call_t *redis_call) {
    ...
    for (midx = 0; midx < redis_call->media->length; midx++) {
        ...
        if (media->tag && media->tag->tag && media->tag->other_tag && media->tag->other_tag->tag &&
            ...
            call_get_mono_dialogue(c, media->tag->tag, media->tag->other_tag->tag, media->tag->viabranch, NULL);
            updated = 1;
        }
    }
...
//    if (&media->codecs.codec_prefs.length != &m->codecs.codec_prefs.length) {
//        rlog(LOG_INFO, "['" STR_FORMAT_M "'] media %u: replacing %d local codec prefs recv with %d remote codec prefs",
//             STR_FMT_M(&m->call->callid), m->unique_id, &m->codecs.codec_prefs.length,
//             &media->codecs.codec_prefs.length);
...
static void restore_thread(void *call_p, void *ctx_p) {
	...
	json_parse_end_restore_call(r, &callid, ctx->foreign);
