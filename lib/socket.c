...
int create_foreign_socket(socket_t *r, int type, unsigned int port, const sockaddr_t *sa, const str *label) {
    ZERO(*r);
    r->is_foreign = 1;
    r->family = sa->family;
    r->fd = -1; // Make sure no one tries to close a real fd

    if (__socket(r, type, r->family)) {
        ilog(LOG_ERROR, "Open socket fail, fd=%d", r->fd);
        goto fail;
    }
    ...
    if (r->family->bind(r, port, sa)) {
        ilog(LOG_ERROR, "Open socket fail, fd=%d, port=%d", r->fd, port);
        goto fail;
    }
